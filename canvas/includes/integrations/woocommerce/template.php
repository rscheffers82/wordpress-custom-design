 ...
 * @return void
 */
if ( ! function_exists( 'woo_nav_cart_link' ) ) {
function woo_nav_cart_link () {
?>
	<a class="cart-contents" href="<?php echo esc_url( WC()->cart->get_cart_url() ); ?>" title="<?php _e( 'View your shopping cart', 'canvas' ); ?>">
		<?php echo wp_kses_data( WC()->cart->get_cart_total() ); ?> <span class="count"><?php echo wp_kses_data( sprintf( _n( '%d item', '%d items', WC()->cart->get_cart_contents_count(), 'canvas' ), WC()->cart->get_cart_contents_count() ) );?></span>
	</a>
<?php
} // End woo_nav_cart_link()
}

if ( ! function_exists( 'woo_add_header_cart' ) ) {
function woo_add_header_cart () {
	global $woocommerce;
